...
import { readFirebaserc, withAlias, writeFirebaserc } from './lib/firebaserc'
import { confirm } from './lib/prompt'
import { capture, run } from './lib/shell'

const DEFAULT_REGION = 'australia-southeast1'
const SERVICES = ['firestore.googleapis.com', 'sheets.googleapis.com', 'apikeys.googleapis.com']
const SHEETS_KEY_NAME = 'appliance-checks-sheets'
...
function sheetsKeyName(projectId: string): string {
  return capture('gcloud', [
    'services', 'api-keys', 'list', `--project=${projectId}`,
    `--filter=displayName=${SHEETS_KEY_NAME}`, '--format=value(name)',
  ]).stdout
}
...
function ensureAlias(env: string, projectId: string): void {
  step(`.firebaserc alias "${env}"`)
  const rc = readFirebaserc() ?? {}
  const updated = withAlias(rc, env, projectId)
  if (rc.projects?.[env] === projectId) {
