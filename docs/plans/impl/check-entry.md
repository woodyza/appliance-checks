...
- **Decisions taken after review** (phase 2 review fixes):
  - **`sensitive` moved to the leaf (child) routes, plus a global lowercase guard**: vue-router 5.3.1's matcher fails to register the empty-path child at all when the parent record that owns `children` also sets `sensitive: true` (verified against the emulator/dev server: `router.getRoutes()` showed the child registered as an orphaned top-level record with the same path, never matched). Moving `sensitive: true` onto the two leaf child records (`sectionList`, `sectionView`) fixes the match while keeping case-sensitivity for the combined path. A `router.beforeEach` global guard additionally redirects any non-lowercase `slug` param to `/`, as an explicit backstop that doesn't depend on exactly how each record's case-sensitivity is split.
  - **`?check=` is synced to the URL only after the session finishes its initial load**: syncing on every `selectedDate` change (as originally implemented) could write a *transient* default (computed before `brigade`/`previousComplete` had loaded) into the URL; once that value happened to be a legitimate existing Check date, `selectedDate`'s "use the explicit selection if valid" guard locked onto it and never corrected itself once loading finished. `CheckView.vue` now watches `[session.loading, session.selectedDate]` together (with `immediate: true`) and only calls `router.replace` once `loading` is `false`, syncing the final value even when it happens to equal the transient one.
  - **`no-undef: off`** in `eslint.config.js`: the core rule false-positives on ambient DOM lib types (`Event`, `HTMLSelectElement`, `HTMLInputElement`) used only in type positions inside `.vue` `<script setup>` blocks, a known interaction between `vue-eslint-parser` and `@typescript-eslint/parser`. TypeScript itself already catches genuine undefined-variable errors.
  - **The `ready` gate blocks writes, not rendering**: until the stamped version for the selected Check is known, `answer()`/`optIn()` no-op (ADR 0002: a write must never silently move a Frozen Check onto the latest Check Sheet). The UI itself only shows the loading screen before the session has *ever* been ready for a selection (`everReady`, one-way latch); later re-gating (a Check switch, or a version bump right after a write) leaves whatever's already rendered in place rather than swapping it for a spinner, which would close the keyboard and drop in-flight input. Cached stamped versions (the current version, and anything already loaded this session) are set synchronously, with no `await` at all, so this re-gating window is rare in practice.
  - **`App.vue` keys its `<router-view>` on `slug/applianceId`**: navigating directly between two appliances under the same slug matches the same route record, so Vue Router would otherwise reuse the `CheckView` instance (and its stale session) instead of remounting it. The key excludes `sectionId`, so Section navigation within one appliance is untouched.
  - **Choice and written inputs also get the `saving` pending marker** (`.item-select.saving`, `.item-input.saving` in `src/style.css`), matching the one the design already describes generically; originally only Y/N buttons had it, which left no way for a caller (or a test) to tell when a choice/written write had actually landed.
  - **`today()` is a reactive `todayRef`** inside `useCheckSession`, recomputed on `visibilitychange` to visible: a tab left open across a Check Day (or midnight) now picks up the new current Check, default and selector once it's looked at again, rather than staying stuck on the date the session first loaded with. An explicit `?check=` selection is unaffected, since it's checked against the (now up to date) selector list rather than recomputed itself.
